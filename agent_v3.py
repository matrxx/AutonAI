...
If you don't need to use a tool, just respond normally."""
    ...
    def run(self, user_input: str) -> str:
        ...
        # Direct handling for simple calculations
...
function uploadFile() {
    ...
    .then(data => {
        if (data.error) {
            ...
            fileInfo.textContent = `${file.name} uploaded (${data.textLength} characters). The AI can now reference this document.`;
...
function clearConversation() {
    ...
    .then(data => {
        ...
        fileUpload.value = '';
        appendMessage('assistant', 'Conversation and document context cleared.');
    })
    .catch(error => {
        appendMessage('assistant', `Error clearing conversation: ${error.message}`);
    });
}
''')
...
if __name__ == '__main__':
    ...
    print("Starting AI Agent Web Interface on http://127.0.0.1:5000")
    app.run(debug=True)